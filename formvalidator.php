...
define("E_VAL_MINLEN_CHECK_FAILED","Please enter with length more than %d for %s");
...
class FormValidator 
{
	...
	function validate_username($input_value,$max_len,$variable_name,&$default_error_message)
	{
		if(isset($input_value) )
		{
...
$q="SELECT * FROM userdetails WHERE username = '$input_value'";
$unn=mysql_query($q,$con) or die ('Error: '.mysql_error ());
while($row = mysql_fetch_assoc($unn))
  {
  $un=$row['username'];
  }
if($un==$input_value)
			...
			case 'username':
						{
							...
							$max_len = intval($command_value);
							$bret = $this->validate_username($input_value,$max_len,$variable_name,
												$default_error_message);
							break;
						}
